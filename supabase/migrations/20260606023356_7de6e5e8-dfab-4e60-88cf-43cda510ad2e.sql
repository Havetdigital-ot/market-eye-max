...
-- 1) Convert seed_demo_data to SECURITY INVOKER so RLS applies as the caller.
...
-- 2) Lock down Realtime channel subscriptions to per-user topics like "user:<uid>".
ALTER TABLE realtime.messages ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can read own channel" ON realtime.messages;
CREATE POLICY "Users can read own channel"
